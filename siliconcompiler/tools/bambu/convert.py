...
class ConvertTask(TaskSchema):
    ...
    def runtime_options(self):
        ...
        if self.schema().valid("asic", "mainlib"):
            device = self.schema().get("library", self.schema().get("asic", "mainlib"), "tool", "bambu", "device")
            if device:
    ...
    def post_process(self):
        ...
        shutil.copy2(f'{self.design_topmodule}.v', os.path.join('outputs', f'{self.design_topmodule}.v'))
        ...
        with sc_open(log_file) as log:
            for line in log:
                ...
                if area_match:
                    self.record_metric("cellarea", float(area_match[0]), log_file, source_unit='um^2')
                if fmax_match:
