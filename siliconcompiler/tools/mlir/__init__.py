'''
MLIR is a compiler infrastructure, part of the LLVM project, built around
reusable dialects and progressive lowering between them. It is the substrate
the SODA Synthesizer's front end (:ref:`soda-opt <tool-soda>`) is written
against, and the tools in this module are the upstream LLVM binaries that the
SODA flow drives directly:

* ``mlir-opt`` -- runs pass pipelines over a ``.mlir`` file.
* ``mlir-translate`` -- translates the LLVM dialect into LLVM IR (``.ll``).
* ``clang`` -- compiles a C support module to LLVM IR.
* ``llvm-link`` -- links LLVM IR modules, used to supply the runtime helpers
  the bufferized code calls.

Because soda-opt is an out-of-tree MLIR project that links against MLIR's C++
libraries, both have to come from the same llvm-project revision. The install
script pins that revision; a distribution ``mlir-tools`` package will generally
not match it.
...
class MLIRTask(Task):
    '''Common behavior for the LLVM/MLIR command line tools.

    Every tool in this module is an LLVM ``cl::opt`` program, so they share a
    ``--version`` format, and every one of them is a file-in/file-out filter
    that either continues a chain of MLIR nodes or starts one from the design's
    filesets.
    '''
    ...
    def parse_version(self, stdout: str) -> str:
        ...
        # the MLIR pass names and dialects are versioned with it.
    ...
    def _record_output_lines(self, path: str) -> Optional[int]:
        ...
        what its passes did to it -- an unrolled kernel is visibly bigger than
        the one that went in -- so every task that writes one reports it. The
        argument-description files a task may write alongside the module are not
        modules and are not counted.

        This is duplicated in :class:`~siliconcompiler.tools.soda.SODATask`
        rather than shared, for the reason that class's docstring gives: the two
        task families are deliberately unrelated types, which is also why
...
def render_pipeline_options(options: List[Tuple[str, Optional[object]]]) -> str:
    '''Renders MLIR pass-pipeline options into the ``flag a=1 b=2`` form the
    ``PassPipelineOptions`` parser expects.
