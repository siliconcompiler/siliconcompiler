...
class RepairDesignTask(APRTask, OpenROADSTAParameter, OpenROADRSZDRVParameter):
    def __init__(self):
        ...
        self.add_parameter("ifp_tie_separation", "float", "maximum distance between tie high/low cells in microns", defvalue=0, unit="um")

        self.add_parameter("rsz_buffer_inputs", "bool", "true/false, when true enables adding buffers to the input ports", defvalue=False)
        self.add_parameter("rsz_buffer_outputs", "bool", "true/false, when true enables adding buffers to the output ports", defvalue=False)
