...
from siliconcompiler.tools.openroad._apr import OpenROADSTAParameter, OpenROADGRTParameter, OpenROADANTParameter
...
class AntennaRepairTask(APRTask, OpenROADSTAParameter, OpenROADGRTParameter, OpenROADANTParameter):
    def __init__(self):
        ...
        self.add_parameter("ant_check", "bool", "true/false, flag to indicate whether to check for antenna violations", defvalue=True)
        self.add_parameter("ant_repair", "bool", "true/false, flag to indicate whether to repair antenna violations", defvalue=True)
    ...
    def pre_process(self):
        ...
        if not self.get("var", "ant_check"):
            ...
            self.logger.warning(f'{self.step}/{self.index} will be skipped since antenna repair is disabled.')
