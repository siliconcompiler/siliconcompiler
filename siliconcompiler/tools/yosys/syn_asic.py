...
class ASICSynthesis(YosysTask):
    ...
    def _get_clock_period(self):
        ...
        clock_units_multiplier = self.schema().get("library", mainlib, field="schema").get("tool", "yosys", "abc_clock_multiplier") / 1000
        period = None
        for lib, fileset in self.schema().get_filesets():
            if not lib.valid("fileset", fileset, "file", "sdc"):
                continue
            for sdc in lib.find_files("fileset", fileset, "file", "sdc"):
                lines = []
