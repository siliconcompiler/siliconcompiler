...
class ChecklistSchema(NamedSchema):
    ...
    def check(self, runnable, items=None, check_ok=False, require_reports=True, logger=None):
        '''
        ...
        for item in items:
            if item not in self.getkeys():
                self.logger.error(f'{item} is not a check in {self.name()}.')
                error = True
                continue
            ...
            for criteria in all_criteria:
                ...
                for job, step, index in tasks:
                    ...
                    for report in reports:
                        ...
                        report = os.path.relpath(report, runnable.cwd)
                        print(report)
