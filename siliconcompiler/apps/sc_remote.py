...
"""
    ...
    class RemoteProject(Project):
        def __init__(self):
            ...
            self._add_commandline_argument("server", "str",
                                           "address of server for configure (only valid with -configure)")
            self._add_commandline_argument("add", "[dir]",
                                           "path to add to the upload whitelist (only valid with -configure)")
            self._add_commandline_argument("remove", "[dir]",
                                           "path to remove from the upload whitelist (only valid with -configure)")
            self._add_commandline_argument("list", "bool",
                                           "print the current configuration (only valid with -configure)")
    ...
    if exclusive_count > 1:
        remote.logger.error(f'Error: {", ".join(["-"+e for e in exclusive])} are mutually exclusive')
        return 1
