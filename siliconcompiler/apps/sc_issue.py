...
"""  # noqa E501
    ...
    class IssueProject(Project):
        def __init__(self):
            ...
            self._add_commandline_argument("hash_files", "bool", "flag to hash the files in the schema before generating the manifest")
            self._add_commandline_argument("file", "file", "filename for the generated testcase")
            self._add_commandline_argument("add_pdk", "[str]", "pdk to include in the testcase, if not provided all libraries will be added according to the -exclude_pdks flag")
            self._add_commandline_argument("add_library", "[str]", "library to include in the testcase, if not provided all libraries will be added according to the -exclude_libraries flag")
            self._add_commandline_argument("exclude_libraries", "bool", "flag to ensure libraries are excluded in the testcase")
            self._add_commandline_argument("exclude_pdks", "bool", "flag to ensure pdks are excluded in the testcase")

            # TODO port add dep func
            # self._add_commandline_argument("add_dep", "[str]", "dependency to load before running", "-add_dep <str>")
    ...
    if not issue.get("cmdarg", "run"):
        ...
        project.set("arg", "step", issue.get("arg", "step"), clobber=False)
        project.set("arg", "index", issue.get("arg", "index"), clobber=False)
