...
class Chip(SchemaConsumer):
    ...
    def __init__(self, design, loglevel=None):
        ...
        # Local variables
        # Call 'dirname' twice to go up one level, so the 'scroot' path refers to the
        # top-level 'siliconcompiler/' directory instead of 'siliconcompiler/core/'.
        self.scroot = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self._error = False
        try:
            self.cwd = os.getcwd()
...
If you are sure that your working directory is valid, try running `cd $(pwd)`.""", fatal=True)
    ...
    def use(self, module):
        ...
        if isinstance(new_schema, PDK):
            self._merge_manifest(new_schema.schema)
