...
#       Package libclang-11.0-dev missing?
...
clang_cxx_probe() {
    echo '#include "clang/Basic/Diagnostic.h"' |
        clang++-11 -x c++ -std=c++17 -fsyntax-only \
            -I"$(llvm-config-11 --includedir)" - 2> /dev/null
}

if command -v clang++-11 > /dev/null 2>&1 &&
        command -v llvm-config-11 > /dev/null 2>&1 &&
        ! clang_cxx_probe; then
    clang_gcc=$(clang++-11 -v -x c++ /dev/null -fsyntax-only 2>&1 |
    ...
    if ! clang_cxx_probe; then
        echo "WARNING: clang++-11 cannot compile clang's own C++ headers, so" >&2
        echo "         configure will report the clang plugin as unsupported." >&2
        echo "         It is using the libstdc++ from GCC ${clang_gcc}; if that is" >&2
        echo "         newer than clang 11 supports, removing that GCC's -dev" >&2
