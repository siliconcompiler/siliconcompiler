...
class Project(PathSchemaBase, CommandLineSchema, BaseSchema):
    ...
    def _get_write_depgraph_extra(self) -> Tuple[Dict[str, Set[str]], Dict[str, Dict[str, str]]]:
        """
        Provides extra graph nodes and styles for the dependency graph.

        This method can be overridden by subclasses to add additional nodes or
        customize styles in the dependency graph generated by `write_depgraph()`.

        Returns:
            Tuple[Dict[str, Set[str]], Dict[str, Dict[str, str]]]: A tuple containing:
                - A dictionary representing extra graph edges.
                - A dictionary mapping node IDs to their style attributes.
        """
        return {}, {}
    ...
    def write_depgraph(self, filename: str,
        ...
        Shows the project's design and its configured filesets, with any
        aliases resolved and reflected in the graph.
        ...
        '''
        # Rebuild alias map: (src_lib_name, src_fs) -> (dst_lib_obj_or_None, dst_fs_or_None)
        alias = {}
        for src_lib, src_fileset, dst_lib, dst_fileset in self.option.get_alias():
            if src_lib == dst_lib and src_fileset == dst_fileset:
                continue
            if dst_lib:
                ...
                dst_obj = None
            alias[(src_lib, src_fileset)] = (dst_obj, dst_fileset or None)

        root = self.name
        primary_filesets = self.option.get_fileset()

        def node_id(lib_name, fs_name):
            return f"{lib_name}/{fs_name}"

        graph: Dict[str, Set[str]] = {
            root: set(node_id(root, fs) for fs in primary_filesets)
        }
        node_styles: Dict[str, Dict[str, str]] = {
            root: {'shape': 'box'}
        }

        for lib, fileset in self.get_filesets():
            nid = node_id(lib.name, fileset)

            edges: Set[str] = set()
            for dep_lib_name, dep_fs in lib.get("fileset", fileset, "depfileset"):
                key = (dep_lib_name, dep_fs)
                if key in alias:
                    dep_obj, new_fs = alias[key]
                    if dep_obj is None:
                        deleted_nid = node_id(dep_lib_name, dep_fs)
                        edges.add(deleted_nid)
                        if deleted_nid not in node_styles:
                            graph[deleted_nid] = set()
                            node_styles[deleted_nid] = {
                                'shape': 'rectangle',
                                'color': '#808080',
                                'text': f"{dep_lib_name}/{dep_fs}"
                            }
                        continue
                    dep_lib_name = dep_obj.name
                    if new_fs:
                        dep_fs = new_fs
                edges.add(node_id(dep_lib_name, dep_fs))

            graph[nid] = edges

        extra_graph, extra_styles = self._get_write_depgraph_extra()
        for node, edges in extra_graph.items():
            if node not in graph:
                graph[node] = set()
            graph[node].update(edges)
        node_styles.update(extra_styles)

        DependencySchema._write_depgraph(
            filename, root, graph, node_styles=node_styles,
