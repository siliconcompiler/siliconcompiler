...
class Project(PathSchemaBase, CommandLineSchema, BaseSchema):
    ...
    def check_manifest(self) -> bool:
        ...
        if design and filesets:
            # Verify all filesets exist before checking patches
            design_obj = self.design
            filesets_exist = all(design_obj.has_fileset(fs) for fs in filesets)
            ...
            if filesets_exist:
                try:
                    ...
                    # Alias validation failed - error already logged earlier
                    pass
    ...
    def _summary_headers(self) -> List[Tuple[str, str]]:
        ...
        patches = []
        if filesets and self.design:
            # Verify all filesets exist before collecting patches
            filesets_exist = all(self.design.has_fileset(fs) for fs in filesets)
