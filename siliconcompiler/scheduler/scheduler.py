        ...
        try:
            for step, index in self.__flow_runtime.get_nodes():
                ...
                for in_step, in_index in excluded:
                    failed = NodeStatus.is_error(
                        self.__record.get('status', step=in_step, index=in_index))
                    if failed and self.__project.option.get_continue(step=in_step,
                                                                     index=in_index):
                        # The excuse Task._is_input_excused applies, read straight
                        # from the schema: that method needs a runtime context and
                        # this runs before any node has one.
                        continue
                    if failed or not os.path.isdir(os.path.join(
