...
def _setup_node(chip, step, index, flow=None):
    ...
    tool_schema = chip.schema.get("tool", tool, field='schema')
    tool_schema.set_runtime(chip)
    ...
    def user_setup(self):
        ...
        return setup_step(chip)

    setattr(tool_schema, "setup", types.MethodType(user_setup, tool_schema))

    # Run node setup.
    try:
        chip.logger.info(f'Setting up node {step}{index} with {tool}/{task}')
        setup_ret = tool_schema.setup()
...
def _check_version(chip, reported_version, tool, step, index):
    def normalize_version(self, version):
        ...
        if not normalize_version:
            return version
        return normalize_version(version)

    tool_schema = chip.schema.get("tool", tool, field='schema')
    tool_schema.set_runtime(chip)
    setattr(tool_schema, "normalize_version", types.MethodType(normalize_version, tool_schema))
    return tool_schema.check_exe_version(reported_version)
...
def _write_task_manifest(chip, tool, path=None, backup=True):
    ...
    if not path:
        path = '.'
    tool_schema.write_task_manifest(path, backup=backup)
...
def _setup_workdir(chip, step, index, replay):
    ...
    tool_schema = chip.schema.get("tool", tool, field="schema")
    tool_schema.setup_work_directory(workdir, remove_exist=not replay)
...
def _get_run_env_vars(chip, tool, task, step, index, include_path):
    tool_schema = chip.schema.get("tool", tool, field="schema")
    tool_schema.set_runtime(chip)
    return tool_schema.get_runtime_environmental_variables(include_path=include_path)
...
def _makecmd(chip, tool, task, step, index, script_name='replay.sh', include_path=True):
    ...
    def user_runtime_options(self):
        runtime_options = getattr(chip._get_task_module(step, index), 'runtime_options', None)
        if not runtime_options:
            return []
        return runtime_options(chip)

    setattr(tool_schema, "runtime_options", types.MethodType(user_runtime_options, tool_schema))

    cmdlist = tool_schema.get_runtime_command()
    ...
    tool_schema.generate_replay_script(script_name, work_dir, include_path=include_path)
...
def _run_executable_or_builtin(chip, step, index, version, toolpath, workdir, run_func=None):
    ...
    tool_schema.generate_replay_script('replay.sh', workdir)
    ...
    if chip.get('option', 'quiet', step=step, index=index):
        chip.logger._console.setLevel(logging.ERROR)

    from siliconcompiler.tool import TaskTimeout, TaskTerminate
    try:
        retcode = tool_schema.run_task(
            workdir,
            quiet, chip.get('option', 'loglevel', step=step, index=index),
...
def _post_process(chip, step, index):
    ...
    def post_process(self):
        post_process = getattr(chip._get_task_module(step, index), 'post_process', None)
        if post_process is None:
            return
        return post_process(chip)

    tool_schema = chip.schema.get("tool", tool, field='schema')
    tool_schema.set_runtime(chip)
    setattr(tool_schema, "post_process", types.MethodType(post_process, tool_schema))
    try:
        tool_schema.post_process()
    except Exception as e:
...
def _pre_process(chip, step, index):
    ...
    def pre_process(self):
        pre_process = getattr(chip._get_task_module(step, index), 'pre_process', None)
        if pre_process is None:
            return
        return pre_process(chip)

    tool_schema = chip.schema.get("tool", tool, field='schema')
    tool_schema.set_runtime(chip)
    setattr(tool_schema, "pre_process", types.MethodType(pre_process, tool_schema))
    try:
        tool_schema.pre_process()
    except Exception as e:
...
def _check_tool_version(chip, step, index, run_func=None):
    ...
    def parse_version(self, stdout):
        parse_version = getattr(chip._get_tool_module(step, index), 'parse_version', None)
    ...
    tool_schema = chip.schema.get("tool", tool, field='schema')
    tool_schema.set_runtime(chip)
    setattr(tool_schema, "parse_version", types.MethodType(parse_version, tool_schema))
    exe = tool_schema.get_exe()
    version = tool_schema.get_exe_version()
    if not chip.get('option', 'novercheck', step=step, index=index) and \
            not _check_version(chip, version, tool, step, index):
        _haltstep(chip, flow, step, index)
    return exe, version
