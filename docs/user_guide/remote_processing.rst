...
For security reasons, only a subset of the full Schema parameters is currently supported. T Values for disallowed keypaths will be silently dropped by the server. In addition, note that the target() function will be re-run on the server, to ensure targets are set up with their default configurations. The following table documents the list of supported remote parameters and any associated restrictions.
