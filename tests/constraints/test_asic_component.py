...
def test_set_get_partname(component_constraint):
    ...
    with pytest.raises(ValueError, match=r"^a partname is required$"):
...
def test_set_get_halo(component_constraint):
    ...
    with pytest.raises(ValueError, match=r"^x must be a positive number$"):
        component_constraint.set_halo(-1.0, 1.0)
    with pytest.raises(ValueError, match=r"^y must be a positive number$"):
        component_constraint.set_halo(1.0, -1.0)
...
def test_add_component(component_constraints_collection):
    ...
    with pytest.raises(TypeError, match=r"^component must be a component constraint object$"):
        component_constraints_collection.add_component("not_a_component")

    # Test adding component without a name
    no_name_comp = ASICComponentConstraint()
    with pytest.raises(ValueError, match=r"^component constraint must have a name$"):
...
def test_get_component(component_constraints_collection):
    ...
    # Get non-existent
    with pytest.raises(LookupError, match=r"^non_existent_comp is not defined$"):
...
def test_make_component(component_constraints_collection):
    ...
    with pytest.raises(LookupError, match=r"^made_comp constraint already exists$"):
        component_constraints_collection.make_component("made_comp")

    # Test empty name
    with pytest.raises(ValueError, match=r"^component name is required$"):
...
def test_remove_component(component_constraints_collection):
    ...
    with pytest.raises(LookupError, match=r"^to_remove_comp is not defined$"):
...
def test_timing_constraint_copy_component_samename():
    schema = ASICComponentConstraints()
